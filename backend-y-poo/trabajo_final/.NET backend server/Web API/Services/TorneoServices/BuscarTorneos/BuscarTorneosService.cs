...
namespace Trabajo_Final.Services.TorneoServices.BuscarTorneos
{
    public class BuscarTorneosService : IBuscarTorneosService
    {
        ...
        public async Task<IList<TorneoInscriptoDTO>> BuscarTorneosInscriptos(int id_jugador)
        {
            //torneos en fase registro a los que el jugador se inscribió
            IEnumerable<Torneo> torneos_inscriptos =
                await torneoDAO.BuscarTorneosInscriptos(id_jugador);

            //series
            IEnumerable<Serie_Habilitada> series_habilitadas =
                await torneoDAO.BuscarSeriesDeTorneos(torneos_inscriptos);

            //estado del jugador en cada inscripcion
            IEnumerable<Jugador_Inscripto> jugador_inscripto_en_torneos =
                await torneoDAO.BuscarJugadorInscripto(id_jugador, torneos_inscriptos);

            //mazos de carta del jugador en cada torneo
            IEnumerable<Carta_Del_Mazo> cartas_mazo =
                await cartaDAO.BuscarMazosInscriptos(
                    id_jugador,
                    torneos_inscriptos.Select(torneo => torneo.Id).ToList());

            //armar DTO
            IList<TorneoInscriptoDTO> result = new List<TorneoInscriptoDTO>();

            foreach (Torneo torneo in torneos_inscriptos)
            {
                ...
                IList<int> mazo = new List<int>();
                foreach(Carta_Del_Mazo carta in cartas_mazo)
                {
                    if (carta.Id_torneo == torneo.Id)
                        mazo.Add(carta.Id_carta);
                }

                bool aceptado =
                    jugador_inscripto_en_torneos
                        .FirstOrDefault(j => j.Id_torneo == torneo.Id)
                        .Aceptado;
                        


                result.Add(new TorneoInscriptoDTO()
                {
                    Id_torneo = torneo.Id,
                    Fecha_hora_inicio = torneo.Fecha_hora_inicio,
                    Fecha_hora_fin = torneo.Fecha_hora_fin,
                    ...
                    Pais = torneo.Pais,
                    Fase = torneo.Fase,
                    Max_cantidad_rondas = torneo.Max_cantidad_rondas,
                    Series_habilitadas = series.ToArray(),
                    Id_cartas_mazo = mazo.ToArray(),
                    Inscripcion_aceptada = aceptado
                });
